		...
		<button
			...
			className={clsx(
				...
				isActive
					...
					: "bg-white border-gray-200 hover:border-purple-200 hover:shadow-sm hover:bg-gray-50"
			)}>
			<span
				className={clsx(
...
export default function Sidebar({
	...
	return (
		<aside className="w-80 bg-white border-r border-gray-200 flex flex-col shrink-0">
			...
			<div
				id="sidebar-list"
				className="flex-1 scroll-container p-3 space-y-2">
				<button
			...
			<div className="p-4 border-t border-gray-200 bg-white grid grid-cols-3 gap-2">
				{Object.entries(QUESTION_TYPES).map(([key, config]) => (
					<button
						...
						<div
							className={`w-8 h-8 rounded-full ${config.bg} ${config.color} flex items-center justify-center group-hover:scale-110 transition-transform`}>

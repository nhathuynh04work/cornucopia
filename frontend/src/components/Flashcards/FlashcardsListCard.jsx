...
export default function FlashcardsListCard({ list }) {
  return (
    <Link
      ...
      className="group relative block bg-white rounded-2xl h-32 border border-gray-100 shadow-sm overflow-hidden transition-all hover:shadow-md hover:translate-y-[-2px] focus:outline-none focus:ring-2 focus:ring-purple-500 p-4"
    >
      {/* Info */}
      <div className="flex flex-col justify-between h-full">
        <h2
          className="text-sm font-semibold text-gray-900 line-clamp-2 group-hover:text-purple-600 transition-colors"
          title={list.title || "Danh sách chưa đặt tên"}
        >
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-1 text-gray-500 text-xs">
            ...
            <span>{list._count?.flashcards || 0} thẻ</span>
          </div>
        </div>
      </div>

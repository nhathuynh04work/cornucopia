    ...
    <div className="bg-[#eaf2ff] rounded-2xl shadow-md p-10 text-center w-[400px] border border-[#d9e4ff]">
      ...
      {!isExitedEarly ? (
        <>
          <p className="text-lg mb-2">
            ✅ Được: <span className="font-semibold text-green-600">{known.length}</span>
          </p>
          <p className="text-lg mb-2">
            ❌ Không được: <span className="font-semibold text-red-500">{unknown.length}</span>
          </p>
      ...
      {studyDuration && (
        <p className="text-lg mt-3 text-gray-700">
          🕒 Thời gian học: <span className="font-semibold">{studyDuration}</span>
        </p>
      )}

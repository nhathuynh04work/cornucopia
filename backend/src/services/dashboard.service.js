...
export async function getUserOverallStats({ userId }) {
	...
	const [
		...
		// Enrolled Courses (All courses) - Filtered for PUBLIC lessons/modules
		...
		// Recent Lesson Activity (Limit 5)
...
export async function getCreatorOverallStats({ userId }) {
	const currentUserId = Number(userId);

	// Refactored to fetch only STATS and RECENT CONTENT
	const [
		courseCount,
		deckCount,
		testCount,
		postCount,
		enrollments, // Still needed for "Total Students" stat
		recentCourses,
		recentDecks,
		recentTests,
		recentPosts,
	...
	return {
		...
		// Charts data removed from here
	};
}

export async function getCreatorChartData({ userId, chartType, timePeriod }) {
...
async function getAdminMonthlyUserGrowth({ months }) {
	let startDate = new Date("2000-01-01"); // Default to 'ALL'
	if (months !== "ALL") {
		const today = new Date();
...
export async function getAdminOverallStats() {
	const [
		totalUsers,
		totalCourses,
		totalDecks,

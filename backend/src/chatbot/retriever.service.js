...
export async function retrieve(query, k = 6, filters = {}) {
  ...
  // ===== BLOG search
  let blogRows = await ragRepo.searchChunksFT(q, limit, topicIds);
  if (!blogRows.length && tokens.length >= 2) {
    blogRows = await ragRepo.searchChunksFT_OR(tokens, limit, topicIds);
  }
  if (!blogRows.length && tokens.length >= 2) {
    const patterns = tokens.map((t) => `%${t}%`);
    blogRows = await ragRepo.searchChunksILIKE(patterns, limit, topicIds);
  }
  ...
  // ===== Merge & cap
  const merged = [
    ...blogRows.map((r) => ({ source: "blog", ...r })),
    ...courseRows, // đã có source: "course"
  ];

...
export async function retrieve(query, k = 6, filters = {}) {
  ...
  // ===== TEST search
  let testRows = await ragRepo.searchTestChunksFT(q, limit);
  if (!testRows.length && tokens.length >= 2) {
    testRows = await ragRepo.searchTestChunksFT_OR(tokens, limit);
  }
  if (!testRows.length && tokens.length >= 2) {
    const patterns = tokens.map((t) => `%${t}%`);
    testRows = await ragRepo.searchTestChunksILIKE(patterns, limit);
  }

  // ===== Merge & cap
  const merged = [...blogRows, ...courseRows, ...testRows];
